...
const Product = require("../models/saleoutProducts.model");
...
exports.getAllProducts = async (req, res) => {
  ...
  try {
    ...
    const saleoutProducts = await Product.find({ shopId: { $in: shops._id } }).lean();

    // Map product prices to double and include virtual id
    const productsWithDoublePrices = saleoutProducts.map(product => {
      return {
        ...product,
    ...
    console.error("Error fetching saleout products details:", error);
    res.status(500).json({
      status: "error",
...
exports.deleteProduct = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ status: "success", message: "Product deleted successfully" });
      ...
      return res.status(404).send({ status: "error", message: `Invalid Product ID- Product not deleted` });
